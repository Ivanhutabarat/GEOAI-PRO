...
export default function MeteorologyModule() {
  ...
  const lowestPressure = Math.min(...currentStation.hourlyData.map(d => d.pressure));
  const triggerStormWarning = lowestPressure < pressureThreshold;
            ...
            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={currentStation.hourlyData}>
                  ...
                  <YAxis yAxisId="right" orientation="right" stroke="#00b4ff" fontSize={10} name="Pressure (mb)" label={{ value: 'Atmospheric Pressure (mb)', angle: 90, position: 'insideRight', fill: '#00b4ff', fontSize: 9 }} />
                  <Tooltip contentStyle={{ backgroundColor: '#111', borderColor: '#333', color: '#fff' }} />
                  <Legend verticalAlign="top" height={24} />
                  <Line yAxisId="left" type="monotone" dataKey="temp" stroke="#FF5722" strokeWidth={2.5} name="Air Temperature (°C)" />
                  <Line yAxisId="right" type="monotone" dataKey="pressure" stroke="#00B4FF" strokeWidth={2} name="Pressure (mb)" />
