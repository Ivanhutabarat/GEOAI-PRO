        ...
        {dimensionMode === '3D' && <div className="fixed inset-0 z-50 md:left-56 top-12 bg-black flex p-6"><Fallback3D /></div>}
          ...
          <div className="geo-card">
            ...
            <div className="h-56 w-full">
              <DebugDump data={meteorologyData} />
<ResponsiveContainer width="100%" height="100%">
                <AreaChart data={forceMapData(meteorologyData || [])}>
                  <defs>
                  ...
                  <YAxis yAxisId="left" stroke="#ff5722" fontSize={10} name="Pressure (mb)" label={{ value: 'Pressure (mb)', angle: -90, position: 'insideLeft', fill: '#ff5722', fontSize: 9 }} domain={['dataMin - 5', 'dataMax + 5']} />
                  <YAxis yAxisId="right" orientation="right" stroke="#00b4ff" fontSize={10} name="Rainfall (mm/h)" label={{ value: 'Rainfall (mm/h)', angle: 90, position: 'insideRight', fill: '#00b4ff', fontSize: 9 }} />
                  <Tooltip contentStyle={{ backgroundColor: '#111', borderColor: '#333', color: '#fff' }} />
                  <Legend verticalAlign="top" height={24} />
                  <Area yAxisId="right" type="monotone" dataKey="rainfall" stroke="#00B4FF" fillOpacity={1} fill="url(#colorRain)" name="Rainfall Rate (mm/h)" />
                  <Line yAxisId="left" type="monotone" dataKey="pressure" stroke="#FF5722" strokeWidth={3} dot={{ stroke: '#FF5722', strokeWidth: 2, r: 4 }} name="Pressure (mb)" />
                  <Line yAxisId="left" type="monotone" dataKey="windSpeed" stroke="#ffcc00" name="Wind Velocities (km/h)" strokeDasharray="3 3" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
