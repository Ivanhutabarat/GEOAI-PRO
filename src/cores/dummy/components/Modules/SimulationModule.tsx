...
import OmniScienceWidget from './OmniScienceWidget';

export default function SimulationModule() {
  const { apiMode, engine, dimensionMode } = useAppContext();
  const { globalData, activeFileName, rawPayloads } = useGlobalGeoContext();
  ...
  const [activeDrawer, setActiveDrawer] = useState<'feed' | 'swarm' | 'omniScience'>('feed');
    ...
    <div className="flex flex-col h-[calc(100vh-10rem)] space-y-6 bg-[#161617] p-5 border border-[#333] rounded-lg">
      ...
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="col-span-12 md:col-span-5 flex flex-col overflow-hidden bg-black/40 border border-[#222] rounded-lg relative h-full">
          ...
          <div className="flex border-b border-[#222] bg-[#111]/90 shrink-0 font-mono text-[10px] items-center text-center font-bold">
            <button
              id="tab-feed-btn"
            ...
            <button
              id="tab-omniscience-btn"
              onClick={() => setActiveDrawer('omniScience')}
              className={cn(
                "flex-1 py-3 text-center transition-all font-bold tracking-wider relative cursor-pointer flex items-center justify-center gap-2",
                activeDrawer === 'omniScience' 
                  ? "bg-[#18181c] text-[#00e5ff] border-b-2 border-[#00e5ff]" 
                  : "text-[#888] hover:text-[#bbb] hover:bg-[#131315]"
              )}
            >
              <Atom size={12} className={activeDrawer === 'omniScience' ? 'text-[#00e5ff] animate-spin' : ''} style={{ animationDuration: '6s' }} />
              OMNISCIENCE
            </button>
          </div>
                  ...
                  >
                    <AnimatePresence initial={false}>
                      {events.length === 0 && !isRunning && (
                        <div className="flex flex-col items-center justify-center h-full text-[#555] font-mono text-xs space-y-2 py-10">
