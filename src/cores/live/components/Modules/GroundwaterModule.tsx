...
import { groundwaterPayload } from '../../../../data/mocks/groundwater';
...
export default function GroundwaterModule() {
  ...
  const chartData = useMemo(() => {
    if (rawPayloads && rawPayloads.electricalData) {
      const result = processIncomingData(rawPayloads.electricalData);
      if (result && result.data && result.data.length > 0) {
        return result.data;
      }
    }
    return (globalData.electricalData && globalData.electricalData.length > 0) 
      ? globalData.electricalData 
      : groundwaterPayload;
  }, [globalData.electricalData, rawPayloads?.electricalData]);
  ...
  const handleTransmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const finalTransmissivity = compressibility === '' ? 1.0 : parseFloat(compressibility);
      // REMOVED MOCK UPDATE
